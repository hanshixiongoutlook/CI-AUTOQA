...
import org.jenkinsci.plugins.client.utils.CILogger;
...
public class JenkinsHelper {
	private CILogger log = CILogger.getInstance();
		...
		while (builds.contains(building)) {
			try {
				...
				for (BuildInfo info : builds) {
					BuildInfo build = BuildUtils.getBuild(info.getJobName(), info.getNumber());
	...
	public void stopAll() {
		for (BuildInfo info : builds) {
			BuildUtils.stopBuild(info.getJobName(), info.getNumber());
		}
	}
	 ...
	 * @author[hanshixiong]
	 */
	private void postFileBuild(String url, File patch) throws HttpException,
			IOException {
