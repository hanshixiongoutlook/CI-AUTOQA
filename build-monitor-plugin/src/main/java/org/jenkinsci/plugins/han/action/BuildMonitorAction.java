...
public class BuildMonitorAction implements RootAction {
	...
	/**
	 * 静态资源代理. 静态资源放置在/resources/static/目录.
	...
	public void doStatic(StaplerRequest request, StaplerResponse response) {
		...
		try {
			InputStream file = BuildMonitorAction.class
					.getResourceAsStream(STATIC_PATH + request.getRestOfPath());
			// 访问资源不存在，返回404
			if (file == null) {
	...
	public void doHello(StaplerRequest request, StaplerResponse response) {
		try {
			...
			Iterator<Entry<String, String[]>> paramIt = param.entrySet()
					.iterator();
	...
	public void doBuildProgress(StaplerRequest request, StaplerResponse response) {
		...
		List<Job<?, ?>> projects = new ArrayList(filter(
				_instance.getAllItems(), Job.class));
	 ...
	 * 
	 * @param request
	 *            querys:jobName1~buildNum,jobName2~buildNum
	 * @param response
	 * @throws IOException
	 */
	public void doLoadBuild(StaplerRequest request, StaplerResponse response)
			throws IOException {
	...
	public void doNextBuildNumber(StaplerRequest request, StaplerResponse response) {
	}
